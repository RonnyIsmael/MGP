$(document).ready(function () {
    ...
    $("#txtCommand").keyup(function (key) {
        let keyCode = key.which; // recommended to use e.which, it's normalized across browsers
        if (keyCode === 13) {
            sendCommand();
            $(this).val("");
        }
    ...
    $("#btnClearLog").click(function () {
        $("#groupConsole").empty();
        alertInfo("Console has cleared.", async function () {
            await sleep(2000);
...
function sendCommand() {
    let cmd = document.getElementById('txtCommand').value; //OBTENEMOS EL COMANDO ESCRITO EN EL CAMPO INPUT
    let serverName = document.getElementById('mcServerName').value; //OBTENEMOS EL servidor EN EL CAMPO INPUT
    if (cmd !== null) {
        postSend(cmd, serverName);
        logMsg(cmd, " > ", "success");
    } else {
        alert("Error");
    }
...
function postSend(cmd, serverName) {
    $.post("/sendCommand", {
        cmd: cmd,
...
function logMsg(msg, sep, cls) {
    ...
    let newMsgAddToList = "<li class=\"list-group-item list-group-item-" + cls + "\"><span class=\"pull-right label label-" + cls + "\">" + datetime + "</span>&nbsp<strong>" + sep + "</strong>&nbsp " + msg + "<div class=\"clearfix\"></div></li>";

    $("#groupConsole")
        .append(newMsgAddToList);
